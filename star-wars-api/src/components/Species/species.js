...
const Species = () => {
  ...
  const {data, status} = useQuery("species", fetchspecies, {
    onSuccess : () => console.log("Species Data fetched successfully!"),
    ...
    <>
      {status === "success" && ( // Display the heading only when the data is successfully loaded
        <div className='heading'>
          <h3 className='headingtext'>Species</h3>
              ...
              <div className="image"></div>
                <div className='greypart'>
                    <div className="divsection">
                        <img src={specieIcon} alt="#" className='filmReel'></img>
                        <p className='text'>{species.name}</p>
            ...
            <div className="table-view">
              <table>
                ...
                <tbody>
                  ...
                  {data.results.map(species => (
                    <tr key={species.name}>
                      <td className='firstCol'><img src={specieIcon} alt={species.name}></img>{species.name}</td>
